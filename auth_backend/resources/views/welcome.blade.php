...
<head>
    ...
    @else
        <style>
            /* Aquí puedes incluir tus estilos en línea o de Tailwind si no usas Vite */
        </style>
    @endif
</head>
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col">
    ...
    <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow starting:opacity-0">
        <main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row">
            ...
            <div class="bg-[#fff2f2] dark:bg-[#1D0002] relative lg:-ml-px -mb-px lg:mb-0 rounded-t-lg lg:rounded-t-none lg:rounded-r-lg aspect-[335/376] lg:aspect-auto w-full lg:w-[438px] shrink-0 overflow-hidden">
                {{-- Aquí se incluyen los SVG de Light y Dark Mode --}}
                @include('partials.svg-illustration')
    ...
    {{-- Sección de ejemplo para permisos y roles --}}
    @auth
        <div class="mt-6">
            <p class="mb-2">Bienvenido, {{ Auth::user()->name }}.</p>
            @can('crear servicios')
                <a href="{{ route('servicios.create') }}" class="inline-block px-4 py-2 bg-green-500 text-white rounded">Crear servicio</a>
            @endcan
            @role('Admin')
                <p class="mt-2 font-bold text-xl">Panel Admin</p>
                <a href="{{ route('admin.dashboard') }}" class="inline-block px-4 py-2 bg-blue-500 text-white rounded">Ir al Dashboard</a>
            @endrole
        </div>
    @endauth
